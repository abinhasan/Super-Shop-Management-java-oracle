...
public class ImageLoadSignature extends java.applet.Applet {
    ...
    public ImageLoadSignature(JComponent comp) {
        try {
    ...
    private boolean createEmptyBlobRow(String pKey) {
        Utility2.insertUpdateQuery("delete from person_signature where person_id = '" + pKey + "' ", new JButton(), 0);

        try {
            String sql = "insert into person_signature values('" + pKey + "',EMPTY_BLOB())";
    ...
    private BLOB getTempBlob(String pKey) {
        try {
            ResultSet rs = stm.executeQuery("select  signature  from person_signature "
    ...
    public void loadImage(String person_id) {
        ...
        try {
            ResultSet rs = stm.executeQuery("select signature from person_signature where person_id = '" + person_id + "'");
            //
            if (rs.next()) {
